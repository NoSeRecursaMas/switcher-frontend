...
describe('RoomList', () => {
    it('Al cargar el componente se refresca la lista de salas', () => {
        ...
        render(<RoomList isUserLoaded={true} selectedRoom={1} setSelectedRoom={() => null} refreshRoomList={refreshRoomList} rooms={[]} />);
        expect(refreshRoomList).toHaveBeenCalled();
    });

    it('Se muestra un mensaje personalizado si no hay salas', () => {
        render(<RoomList isUserLoaded={true} selectedRoom={1} setSelectedRoom={() => null} refreshRoomList={() => null} rooms={[]} />);
    ...
    it('Se muestra un mensaje de carga si no se ha cargado el usuario', () => {
        render(<RoomList isUserLoaded={false} selectedRoom={1} setSelectedRoom={() => null} refreshRoomList={() => null} rooms={[]} />);
        expect(screen.getByText('Cargando salas...')).toBeInTheDocument();
    });

    it('Se muestra un mensaje de carga si no se han cargado las salas', () => {
        render(<RoomList isUserLoaded={true} selectedRoom={1} setSelectedRoom={() => null} refreshRoomList={() => null} rooms={undefined} />);
        expect(screen.getByText('Cargando salas...')).toBeInTheDocument();
    });

    it('Se muestran las salas disponibles', () => {
        render(<RoomList isUserLoaded={true} selectedRoom={1} setSelectedRoom={() => null} refreshRoomList={() => null} rooms={rooms} />);
    ...
    it('Se muestra el tag de sala privada/publica correctamente', () => {
        ...
        expect(screen.getByText('Sala de test').parentElement?.querySelector('span')).toHaveTextContent('Pública');
        expect(screen.getByText('Sala privada').parentElement?.querySelector('span')).toHaveTextContent('Privada');
    });
    ...
    it('El usuario no puede seleccionar una sala llena', async () => {
        ...
        render(<RoomList isUserLoaded={true} selectedRoom={1} setSelectedRoom={setSelectedRoom} refreshRoomList={() => null} rooms={rooms} />);
        await user.click(screen.getByText('Sala llena'));
        expect(sendToast).toHaveBeenCalledWith('La sala está llena', 'No puedes unirte a una que ya alcanzó su límite de jugadores', 'warning');
    ...
    it('El usuario puede seleccionar una sala no llena', async () => {
        ...
        render(<RoomList isUserLoaded={true} selectedRoom={3} setSelectedRoom={setSelectedRoom} refreshRoomList={() => null} rooms={rooms} />);
